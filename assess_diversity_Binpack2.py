...
def main(args: Args):
    ...
    # For BinPacking environment, we need to handle the observation shape
    actor = nn.Sequential(
    ...
    obs, _ = envs.reset()
    obs_vec = torch.FloatTensor(obs).to(device)
    ...
    for i in range(args.num_diversity_steps // args.num_envs):
        ...
        # ✅ Patch: override illegal actions before env step
        corrected_actions = actions.clone()
        for k in range(args.num_envs):
            item_size = float(obs[0][k][-1])
            sac_action = actions[k].item()
            num_bins_levels = obs[0][k][:-1]  # length == bag_capacity

            # Check for overflow
            overflow = sac_action + item_size > 9
            ...
            if overflow or nonexistent:
                ...
                valid_actions = [
                    lvl for lvl in range(1, 9)
                    if lvl <= (9 - item_size) and bin_levels[lvl] > 0
                ]
                valid_actions.append(0)  # Opening new bin is always legal

                fallback = random.choice(valid_actions)
                print(f"⚠️ Env {k}: Illegal SAC action {sac_action} (item size {item_size}) → fallback to {fallback}")
                corrected_actions[k] = fallback
