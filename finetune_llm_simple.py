...
def main(args: Args):
    ...
    envs = gym.vector.SyncVectorEnv(
        [
            make_env(
                args.env_id, args.seed + i, max_episode_steps=args.max_episode_steps
            )
    ...
    action_prompt = (
        ...
        " each of a most 6 words. Thoughts may important observations about the current state of the problem,",
        " or aspects to consider in priorization. Do not make more than 10 thoughts since less thoughts"
        " are preferred as long as they suffice to make an optimal choice. Then, return the chosen action"
        " indicated by the token [action] followed by the action in JSON. The available actions are described below:\n\n"
        f"### Action description\n{envs.metadata['action_space_text']}"
    )


    run_id = f"{args.agent}_{args.env_id}__{args.exp_name}__{args.seed}"
    run_id = run_id.replace("meta-llama/Llama", "llama")    
    ...
    for iter in range(iter_start, num_iterations + 1):
        ...
        b_cot_coefs = [[] for _ in range(args.num_envs)]
        ...
        for step in tqdm(
            ...
            for j in range(args.num_envs):
                if not autoreset[j]:
                    ...
                    # # The value does not change, the logprob of the eot token should be 0
                    # b_out_tok_j = F.pad(results["output_tokens"][j], (0, 1), value=tokenizer.eos_token_id)
                    # b_value_j = F.pad(results["values"][j], (0, 1), value=results["values"][j][-1])
                    # b_logprobs_j = args.cot_coef * F.pad(results["logprobs"][j], (0, 1), value=-0.001)

                    # b_value[j].append(b_value_j)
                    # b_output_tokens[j].append(b_out_tok_j)
                    ...
                    b_cot_coef_j = torch.ones(out_tokens.shape)
                    found_act_token = False
                    for k in range(len(out_tokens)- len(act_tokens)):
                        if not found_act_token:
                            # check match
                            if torch.equal(out_tokens[k:k + len(act_tokens)], act_tokens):
                                break
                            else:
                                b_cot_coef_j[k] = args.cot_coef 

                    b_cot_coefs[j].append(b_cot_coef_j)
        ...
        for epoch in tqdm(range(args.update_epochs), desc=f"Iter: {iter},  Optimizing"):
            ...
            for batch in loader:
                ...
                for c in range(num_chunks):
                    ...
                    mb_logprob = batch["logprob"][:, mb] * mb_cot_coef
                    ...
                    new_logprob = mb_cot_coef * new_logprob.gather(-1, gather_ixs).squeeze(-1)
...
if __name__ == "__main__":
    ...
    args.agent += f"__{args.llm}__{args.cot_coef}"
    main(args)
